...
- **v0.8（流式补全 + 待办引导 + 富文本证据）**：① 「你好重复」诊断为旧 bundle/进程叠加角色弱标识的错觉（消息序列实为各一条）；「无流式」真因是推理模型思考期走 reasoning_content 未转发——现 provider→loop→网关→前端全链路转发思考增量，界面实时显示灰斜体💭思考流（浏览器实测 321 思考帧+124 正文帧）；index.html 设 no-cache 防陈旧 bundle；② 侧边栏「Agent 测试」新增红色待办徽标：跨会话人工步骤聚合 API /api/agent/human-todos + 15s 轮询，弹出的待办不再漏填；③ 人工步骤证据升级富文本编辑器（contentEditable+粘贴截图即上传，序列化为有界 Markdown 复用 note 通道，服务端零改动；完成态渲染图文混排）；④ 项目详情页顶部新增全流程 Stepper（工具模板→执行采集→Agent 深测→判定审核→合规报告，点击跳转），回应"系统割裂看不懂流程"；⑤ 布局重构评估与分期方案成文 wiki/10：P1 统一事件流（对话中心式，中等工作量）为消除割裂的核心一刀。server 97 / modules 66 全绿。
