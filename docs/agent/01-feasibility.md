...
| 阶段 | 名称 | 映射条款 | 执行主体 | 产物 |
|---|---|---|---|---|
| A | 前期准备 Onboarding | 否 | Agent 规划 + 人工硬件操作 + 设备工具 | 设备档案、网络拓扑、接入状态（artifacts，clauseId=null） |
...
选设备 + 选法规模块（阶段 C 的条款集合 + 授权工具集）
  ↓
[A 准备] Agent 规划：识别型号/平台 → 给出调试接口开启步骤（短接/暗码/驱动）
         人工步骤卡片（指令 + 完成按钮 + 截图上传）
         产物：设备档案、接入方式 → artifacts
  ↓
[B 收集] Agent 按功能模块规划：nmap/wireshark/EMBA/抓包/截图
...
从测试笔记提炼，平台需满足（一期覆盖大部分）：

| 测试做法 | 平台需要的能力 | 阶段 |
|---|---|---|
| 读官网/文档识别接口、资产 | Agent 联网/读 URL + 条款库检索（skill） | A/B |
| 短接/暗码/驱动等人工硬件操作 | human_instruction 卡片 + 完成确认 + 截图上传 | A |
| 多平台分型（安卓/RTOS）路径分叉 | Agent 按设备档案动态选 plan | A |
| nmap TCP/UDP 扫描 + 截图 | port-check 模组 / 命令工具 + evidence_attach | B/C |
...
- **范围控制**：一期只做"选条款 → 四阶段人机协同 → 审核 → 报告"主链路；不追求覆盖所有工具类型，先用 port-check/命令工具 + 人工证据跑通一个真实模块（建议从 Z6s 的 GEC/网络外部接口 入手）
